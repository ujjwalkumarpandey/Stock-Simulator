...
router.get("/sellStocks", middlewares.verifyUser, async (req, res) => {
  var id = req.query.id;
...
router.post("/sellStock", middlewares.verifyUser, async (req, res) => {
    ...
    if (error) {
      ...
      if (result[0]) {
        if (result[0].units >= unit) {
                ...
                if (units_in_database == unit) {
                  ...
                  con.query(sql3, async (error, result) => {
                    if (error) {
                      ...
                      var sql = `insert into transactionHistory values('${stockId}',${unit},${currentValue},'${formattedDate}','${formattedTime}','${req.user.username}')`;
          ...
          res.redirect(
            `/api/sell/sellStocks?id=${stockId}&error=` +
            encodeURIComponent("Insuffient unit")
          );
        }
      } else {
        res.redirect(
    ...
    if (error) {
      ...
      if (result[0]) {
        ...
        if (gg.localeCompare(password) == 0) {
          ...
          console.log("symbol: " + symbol);
          res.redirect(`/api/sell/autoBuy?id=${symbol2}&error=wrong password`);
        }
      } else {
        console.log("symbol: " + symbol);
        res.redirect(
...
router.post("/autoSellStock", middlewares.verifyUser,async (req, res) => {
  ...
  var units = parseInt(req.body.units);
  var selected_price = parseFloat(req.body.targetPrice);
  // var selected_price2 = parseFloat(req.body.targetPrice2);//lower limit
    ...
    if (error) {
      ...
      if (result[0]) {
        ...
        if (gg.localeCompare(password) == 0) {
          ...
          res.redirect(`/api/sell/autoSell?id=${symbol2}&error=wrong password`);
        }
      } else {
        res.redirect(
